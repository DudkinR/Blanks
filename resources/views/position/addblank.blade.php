@extends('layouts.app')
@section('title', __("Add avalable positions to blank"))
                ...
                <div class="form-group">
                    ...
                    <select name="positions[]" id="positions" class="form-control" multiple>
                        @foreach($positions as $position)
                            <option value="{{$position->id}}" @if(in_array($position->id,$mypositions)) selected @endif > {{$position->abv}} - {{$position->name}}  </option>
                        @endforeach
                    </select>
                </div>
            @endif
